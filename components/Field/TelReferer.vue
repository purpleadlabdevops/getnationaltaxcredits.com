...
export default {
  ...
  methods: {
    ...
    input(e) {
      let arr = this.$parent.phone.replace(/[^\dA-Z]/g, '').replace(/[\s]/g, '').split('');
      if (arr.length > 0) arr.splice(0, 0, '(');
      if (arr.length > 4) arr.splice(4, 0, ') ');
      if (arr.length > 8) arr.splice(8, 0, '-');
      this.$parent.phone = arr.toString().replace(/[,]/g, '');
    },
    inputOld(){
      const phoneNumber = this.$parent.phone.replace(/[^\dA-Z]/g, '').replace(/[\s]/g, '').split('')
      const mask = '(###) ###-####'.split('')
